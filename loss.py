...
def l1_loss(y_pred, y_true, mask_list):
    ...
    return p3 * tf.reduce_mean(tf.abs(y_pred - y_true)) + p4 * tf.reduce_mean(tf.abs(y_pred - y_true) * (1 - mask_list))
...
generator_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, beta_1=0.5, beta_2=0.999, decay=decay)

@tf.function()
def train_step(generator, image_list, mask_image_list, mask_list):  # image为原图 0 - 1.0
    with tf.GradientTape() as tape:
        ...
        perceptual_loss = p2 * (cal_perceptual(outputs, style_targets) + cal_perceptual(comp_outputs, style_targets))


        L1_loss = l1_loss(gen_image_list, image_list, mask_list)
        tvl_loss = p5 * total_variation_loss(comp, mask_list)
